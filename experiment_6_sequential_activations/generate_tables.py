...
"""
    ...
    for stat in latent_stats[:15]:  # Top 15 by effect size
...
"""
    ...
    for stat in latent_stats[:15]:  # Top 15 by correction activation
        label = stat["latent_label"][:40] + "..." if len(stat["latent_label"]) > 40 else stat["latent_label"]
        label = label.replace("_", "\\_").replace("&", "\\&")
...
def main():
    ...
    output_dir = base_dir / "data" / "self-correction" / "tables"
