"""Experiment runner with optional latent ablation.

This script runs ablation experiments, zero-clamping specified latents during
generation. Supports custom output suffixes for distinguishing different
ablation conditions (e.g., detector ablation vs random ablation).
"""
...
# Load environment variables (HF_TOKEN for gated models)
load_dotenv()
...
async def run_experiment(
    ...
    """
    Run a whole experiment on a model, optionally with latent ablation.
    ...
    Args:
        ...
        output_suffix: Custom suffix for output filename (e.g., "_random_ablation_set0").
                      If None, uses "_with_ablation" when ablate_latents is set.
        output_dir: Custom output directory. If None, uses "experiment_results".
    """
    ...
    async def process_results():
        ...
        # Use custom suffix if provided, otherwise default to "_with_ablation" for ablation runs
        if output_suffix is not None:
            suffix = output_suffix
        else:
            suffix = "_with_ablation" if ablate_latents else ""
        
        # Use custom output directory if provided
        results_dir = output_dir if output_dir else "experiment_results"
        final_filename = f"{results_dir}/experiment_results_{short_model_name}_{time.strftime('%Y%m%d_%H%M%S')}{suffix}.json"
    ...
    try:
        ...
        if experiment_result.results_by_feature:
            if output_suffix is not None:
                ...
                suffix = "_with_ablation" if ablate_latents else ""
            results_dir = output_dir if output_dir else "experiment_results"
            new_filename = f"{results_dir}/experiment_results_{short_model_name}_{time.strftime('%Y%m%d_%H%M%S')}{suffix}_TIMEOUT.json"
            result_dict = asdict(experiment_result)
            if ablate_latents:
                result_dict["ablated_latents"] = ablate_latents
                result_dict["num_ablated_latents"] = len(ablate_latents)
